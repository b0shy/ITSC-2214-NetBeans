...
/**
 * A Pyramid Scheme is a particular form of LinkedTree<Person>
 * that has methods for simulating a pyramid scheme scam.
 *
 * @author pmele
 * @version 3/28/2019
 */
public class PyramidScheme extends LinkedTree<Person> {
    ...
    PyramidScheme(Person startElem, double rPrice, double pUp) {
        ...
        paidUp = pUp;
        while (paidUp > 1) {
            paidUp *= .01; //This will convert percentages into 
            //decimals
        }
    }
    ...
    /**
     * TODO Similar to findNode, but rather than returning the node that
     * contains the target, instead returns a list of nodes above the one that
     * contains the target, in low to high level order.
     *
    ...
    private ArrayList<MultiTreeNode<Person>>
            findNodeChain(MultiTreeNode<Person> node,
                    Person target) {

        ArrayList<MultiTreeNode<Person>> ascendants = 
                new ArrayList<MultiTreeNode<Person>>(); 
        ...
        if (node.getElement().equals(target)) {
            ...
            ArrayList<MultiTreeNode<Person>> temp;
            
            for (MultiTreeNode<Person> person : node.getChildren()) {
                
                temp = findNodeChain(person, target);
        ...
        /**
         * Similar to findNode, this will see it the target exists and return a
         * truth value.
         *
         * @param node the current node being examined
         * @param target the person being searched for
         * @param list the array list sent in
         * @return a boolean if the target is found
         */
    private boolean findNodeChain(MultiTreeNode<Person> node,
            Person target, ArrayList<MultiTreeNode<Person>> list) {
        ...
        for (MultiTreeNode<Person> child : node.getChildren()) {
            if (findNodeChain(child, target, list)) {
     ...
     */
    @Override
    public void addChild(MultiTreeNode<Person> recruiter,
            Person recruit) {
     ...
     */
    private void payUp(MultiTreeNode<Person> moneyMaker,
            double moneyMade) {
        ArrayList<MultiTreeNode<Person>> owed
                = whoBenefits(moneyMaker);
        if (owed == null || owed.isEmpty()) {
            //Person at top gets paid all
            moneyMaker.getElement().adjustBalance(moneyMade);
            return;
        }
        //Initial money maker gets payed, with a portion lost to 
        //those above
        moneyMaker.getElement().adjustBalance(
                (1.0 - paidUp) * moneyMade);
        //Each step above takes a share, but passes a portion of 
        //it up
        for (int i = 0; i < owed.size() - 1; i++) {
            moneyMade = paidUp * moneyMade;
            owed.get(i).getElement().adjustBalance(
                    (1.0 - paidUp) * moneyMade);
        }
    ...
    private int collapse(MultiTreeNode<Person> personNode) {
        int underlings = 1;
        ArrayList<MultiTreeNode<Person>> toSever
                = new ArrayList<>();
